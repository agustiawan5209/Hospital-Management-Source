...
$query_row_schedule = "SELECT ts.id, te.first_name, te.last_name, concat(te.first_name, ' ', te.last_name) as doctor_name, ts.available_days, ts.start_time, ts.end_time, ts.message, ts.status, ts.created_at, te.department_name FROM `tbl_schedule` as ts INNER JOIN tbl_employee as te ON ts.doctor_id = te.id  WHERE ts.id='$id'";
if(isset($_REQUEST['save-schedule']))
{
      ...
      $update_query = mysqli_query($connection, "update tbl_schedule set department='$department', doctor_name='$doctor_name', available_days='$days', start_time='$start_time', end_time='$end_time', message='$message', status='$status' where id='$id'");
      if($update_query>0)
      {
